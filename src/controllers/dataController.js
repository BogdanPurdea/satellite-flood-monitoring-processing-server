...
const { fetchNDWI, fetchPolygonNDWI, fetchGridData } = require('../services/dataService.js');
...
const getGridNDWI = async (req, res) => {
    try {
        const { start_date, end_date, coordinates } = req.body;
        ...
        const fastApiUrl = `${config.fastapi.url}${config.fastapi.endpoints.grid_ndwi}`;
        const data = await fetchGridData(start_date, end_date, coordinates, fastApiUrl, 'NDWI');
        res.json(data);
...
const getGridMNDWI = async (req, res) => {
    try {
        const { start_date, end_date, coordinates } = req.body;
        ...
        const data = await fetchGridData(start_date, end_date, coordinates, fastApiUrl, 'MNDWI');
        res.json(data);
